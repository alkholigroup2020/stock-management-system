 ...
 * - Fetches user session on first load
 */
...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  // If user session hasn't been fetched yet, fetch it
  // This ensures we have the latest auth state on first page load
  if (auth.user.value === undefined && !auth.loading.value) {
