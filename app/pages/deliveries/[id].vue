    ...
    <div class="mb-6">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <nav class="flex items-center space-x-2 text-sm text-[var(--ui-text-muted)] ml-10">
            ...
            <span>/</span>
            <span class="text-[var(--ui-text)]">{{ delivery?.delivery_no || 'Loading...' }}</span>
          </nav>
        </div>
    ...
    <div v-else-if="delivery" class="space-y-6">
      ...
      <UCard class="card-elevated">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div>
            <h3 class="text-sm font-semibold text-[var(--ui-text-muted)] uppercase mb-3">
              Delivery Information
            </h3>
            <dl class="space-y-2">
              <div>
                <dt class="text-sm text-[var(--ui-text-muted)]">Delivery Date</dt>
                <dd class="text-sm font-medium text-[var(--ui-text)]">
                  {{ formatDate(delivery.delivery_date) }}
                </dd>
              </div>
              <div>
                <dt class="text-sm text-[var(--ui-text-muted)]">Invoice Number</dt>
                <dd class="text-sm font-medium text-[var(--ui-text)]">
                  {{ delivery.invoice_no || '—' }}
                </dd>
              </div>
              <div v-if="delivery.po">
                <dt class="text-sm text-[var(--ui-text-muted)]">Purchase Order</dt>
                <dd class="text-sm font-medium text-[var(--ui-text)]">
                  {{ delivery.po.po_no }}
                  <UBadge
          ...
          <div>
            ...
            <dl class="space-y-2">
              <div>
                <dt class="text-sm text-[var(--ui-text-muted)]">Name</dt>
                <dd class="text-sm font-medium text-[var(--ui-text)]">
                  {{ delivery.supplier.name }}
                </dd>
              </div>
              <div>
                <dt class="text-sm text-[var(--ui-text-muted)]">Code</dt>
                <dd class="text-sm font-medium text-[var(--ui-text)]">
                  {{ delivery.supplier.code }}
                </dd>
              </div>
              <div v-if="delivery.supplier.contact">
                <dt class="text-sm text-[var(--ui-text-muted)]">Contact</dt>
                <dd class="text-sm font-medium text-[var(--ui-text)]">
          ...
          <div>
            ...
            <dl class="space-y-2">
              <div>
                <dt class="text-sm text-[var(--ui-text-muted)]">Location</dt>
                <dd class="text-sm font-medium text-[var(--ui-text)]">
                  {{ delivery.location.name }}
                  <span class="text-xs text-[var(--ui-text-muted)]">({{ delivery.location.code }})</span>
                </dd>
              </div>
              <div>
                <dt class="text-sm text-[var(--ui-text-muted)]">Period</dt>
                <dd class="text-sm font-medium text-[var(--ui-text)]">
        ...
        <div v-if="delivery.delivery_note" class="mt-6 pt-6 border-t border-[var(--ui-border)]">
          <h3 class="text-sm font-semibold text-[var(--ui-text-muted)] uppercase mb-2">
            Delivery Note
          </h3>
          <p class="text-sm text-[var(--ui-text)]">{{ delivery.delivery_note }}</p>
        </div>
      </UCard>
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead class="border-b border-[var(--ui-border)] bg-zinc-50 dark:bg-zinc-900">
              <tr>
                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[var(--ui-text-muted)]">
                  Item
                </th>
                <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--ui-text-muted)]">
                  Quantity
                </th>
                <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-[var(--ui-text-muted)]">
                  Unit Price
            ...
            <tbody class="divide-y divide-[var(--ui-border)]">
              ...
              >
                <!-- Item -->
                <td class="px-4 py-3">
                  <div class="font-medium text-sm text-[var(--ui-text)]">
                    {{ line.item.name }}
                  </div>
                ...
                <!-- Quantity -->
                <td class="px-4 py-3 text-right text-sm text-[var(--ui-text)]">
                  {{ line.quantity.toFixed(4) }}
                </td>

                <!-- Unit Price -->
                <td class="px-4 py-3 text-right text-sm font-medium text-[var(--ui-text)]">
                ...
                <!-- Line Value -->
                <td class="px-4 py-3 text-right text-sm font-semibold text-[var(--ui-text)]">
          ...
          <div class="flex justify-end">
            <div class="space-y-2 min-w-[300px]">
              <div class="flex justify-between items-center">
                <span class="text-sm text-[var(--ui-text-muted)]">Total Items:</span>
              ...
              <div v-if="hasVarianceLines" class="flex justify-between items-center">
                <span class="text-sm text-[var(--ui-text-muted)]">Items with Variance:</span>
                <span class="text-sm font-medium text-amber-600 dark:text-amber-400">
                  {{ varianceLinesCount }}
                </span>
              </div>
              <div v-if="hasVarianceLines" class="flex justify-between items-center">
                <span class="text-sm text-[var(--ui-text-muted)]">Total Variance:</span>
                <span
                  :class="[
                    'text-sm font-medium',
              ...
              <div class="flex justify-between items-center pt-2 border-t border-[var(--ui-border)]">
                <span class="text-base font-semibold text-[var(--ui-text)]">Total Amount:</span>
                <span class="text-xl font-bold text-[var(--ui-primary)]">
      ...
      <UCard v-if="delivery.ncrs.length > 0" class="card-elevated">
        ...
        <div class="space-y-4">
          <div
            ...
            :key="ncr.id"
            class="rounded-lg border border-[var(--ui-border)] bg-[var(--ui-bg)] p-4 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors cursor-pointer"
          ...
          >
            <div class="flex items-start justify-between">
              <div class="flex-1">
                ...
                <p v-if="ncr.reason" class="text-sm text-[var(--ui-text-muted)] mb-2">
                  {{ ncr.reason }}
                </p>
