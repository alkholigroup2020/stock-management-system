<template>
  <div
    :class="[
      'border-b border-[var(--ui-border)] bg-[var(--ui-bg)]',
      paddingClasses,
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div class="flex-1 min-w-0">
        ...
        <nav v-if="breadcrumbs && breadcrumbs.length > 0" class="mb-2">
          <ol class="flex items-center space-x-2 text-sm text-[var(--ui-text-muted)]">
            <li v-for="(crumb, index) in breadcrumbs" :key="index" class="flex items-center">
              <NuxtLink
                v-if="crumb.to"
                :to="crumb.to"
                class="hover:text-[var(--ui-primary)] transition-colors"
              >
                {{ crumb.label }}
              </NuxtLink>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              <h1
                :class="[
                  'font-bold text-[var(--ui-text)] truncate',
                  titleSizeClasses
                ]"
              >
                {{ title }}
      ...
      <div v-if="$slots.actions || showBackButton" class="flex items-center gap-2 sm:flex-shrink-0">
        <!-- Back Button -->
        <UButton
          v-if="showBackButton"
          variant="ghost"
...
const iconColorClasses = computed(() => {
  return 'w-5 h-5 text-[var(--ui-primary)]'
})
