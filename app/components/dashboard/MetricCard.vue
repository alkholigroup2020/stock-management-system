  ...
  <UCard class="card-elevated">
    ...
    <div v-else class="flex items-start gap-4">
      ...
      <div class="flex-1 min-w-0">
        <p class="text-sm font-medium text-[var(--ui-text-muted)] mb-1">
          {{ label }}
        </p>
        <p class="text-2xl font-bold text-[var(--ui-text)] mb-2">
          {{ value }}
        </p>

        <!-- Trend Indicator -->
        <div v-if="trend" class="flex items-center gap-1 text-sm" :class="trendColorClass">
