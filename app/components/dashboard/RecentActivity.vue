...
<template>
  <UCard class="card-elevated">
    ...
    <template #header>
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <UIcon v-if="icon" :name="`i-lucide-${icon}`" class="w-5 h-5 text-[var(--ui-text-muted)]" />
          <h3 class="text-lg font-semibold text-[var(--ui-text)]">{{ title }}</h3>
        </div>
        <UButton
          v-if="viewAllRoute"
          variant="ghost"
    ...
    <div v-if="loading" class="flex items-center justify-center py-8">
      <UIcon name="i-lucide-loader-circle" class="w-8 h-8 animate-spin text-[var(--ui-text-muted)]" />
    </div>
    ...
    <div v-else class="divide-y divide-[var(--ui-border)]">
      <div
        v-for="item in items"
        :key="item.id"
        class="py-3 cursor-pointer hover:bg-[var(--ui-bg-elevated)] transition-colors rounded-lg px-2 -mx-2"
        ...
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div v-if="item.icon" class="flex-shrink-0 mt-0.5">
              <div class="p-2 rounded-lg bg-[var(--ui-bg)]">
                <UIcon :name="`i-lucide-${item.icon}`" class="w-4 h-4 text-[var(--ui-text-muted)]" />
              </div>
            </div>
