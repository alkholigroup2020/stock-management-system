...
describe('WAC Calculation Utility', () => {
  ...
  describe('calculateWAC - Error Handling', () => {
    it('should throw error for negative current quantity', () => {
      expect(() => calculateWAC(-10, 10.0, 50, 12.0)).toThrow('Invalid currentQty: cannot be negative')
    })

    it('should throw error for negative current WAC', () => {
      expect(() => calculateWAC(100, -10.0, 50, 12.0)).toThrow('Invalid currentWAC: cannot be negative')
    })

    it('should throw error for zero received quantity', () => {
      expect(() => calculateWAC(100, 10.0, 0, 12.0)).toThrow('Invalid receivedQty: must be greater than zero')
    })

    it('should throw error for negative received quantity', () => {
      expect(() => calculateWAC(100, 10.0, -50, 12.0)).toThrow('Invalid receivedQty: must be greater than zero')
    })

    it('should throw error for negative receipt price', () => {
      expect(() => calculateWAC(100, 10.0, 50, -12.0)).toThrow('Invalid receiptPrice: cannot be negative')
    })

    it('should throw error for non-numeric current quantity', () => {
      expect(() => calculateWAC(NaN, 10.0, 50, 12.0)).toThrow('Invalid currentQty: must be a finite number')
    })

    it('should throw error for infinite values', () => {
      expect(() => calculateWAC(Infinity, 10.0, 50, 12.0)).toThrow('Invalid currentQty: must be a finite number')
    })
  ...
  describe('Precision and Rounding', () => {
    it('should maintain precision for 4-decimal quantities', () => {
      ...
      const result = calculateWAC(10.1234, 5.6789, 20.4567, 6.7890)
