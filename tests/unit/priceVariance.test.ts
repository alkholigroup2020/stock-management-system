...
describe('checkPriceVariance', () => {
  describe('Standard Calculations', () => {
    it('should detect price increase variance', () => {
      ...
      expect(result.varianceAmount).toBe(50.00); // 0.5 * 100
      expect(result.actualPrice).toBe(26.00);
      expect(result.expectedPrice).toBe(25.50);
      expect(result.exceedsThreshold).toBe(true); // Default threshold is 0
    });

    it('should detect price decrease variance', () => {
      const result = checkPriceVariance(24.50, 25.00, 50);

      expect(result.hasVariance).toBe(true);
      expect(result.variance).toBe(-0.5);
      expect(result.variancePercent).toBe(-2.00); // (-0.5 / 25.0) * 100
      expect(result.varianceAmount).toBe(-25.00); // -0.5 * 50
      expect(result.actualPrice).toBe(24.50);
      expect(result.expectedPrice).toBe(25.00);
      expect(result.exceedsThreshold).toBe(true);
    });

    it('should detect no variance when prices match', () => {
      const result = checkPriceVariance(25.00, 25.00, 100);

      expect(result.hasVariance).toBe(false);
      expect(result.variance).toBe(0);
      expect(result.variancePercent).toBe(0);
      expect(result.varianceAmount).toBe(0);
      expect(result.actualPrice).toBe(25.00);
      expect(result.expectedPrice).toBe(25.00);
  ...
  describe('Prisma Decimal Support', () => {
    it('should work with Prisma.Decimal inputs', () => {
      const unitPrice = new Prisma.Decimal(26.00);
      const periodPrice = new Prisma.Decimal(25.50);
  ...
  describe('Threshold Configuration', () => {
    it('should respect percentage threshold', () => {
      ...
      const result1 = checkPriceVariance(25.25, 25.00, 100, config);
      expect(result1.hasVariance).toBe(true);
      expect(result1.variancePercent).toBe(1.00);
      expect(result1.exceedsThreshold).toBe(false);

      // 6% variance - should exceed threshold
      const result2 = checkPriceVariance(26.50, 25.00, 100, config);
      expect(result2.hasVariance).toBe(true);
      expect(result2.variancePercent).toBe(6.00);
    ...
    it('should respect absolute amount threshold', () => {
      ...
      const result1 = checkPriceVariance(25.50, 25.00, 100, config);
      expect(result1.varianceAmount).toBe(50.00);
      expect(result1.exceedsThreshold).toBe(false);

      // SAR 150 variance - should exceed threshold
      const result2 = checkPriceVariance(26.50, 25.00, 100, config);
      expect(result2.varianceAmount).toBe(150.00);
      expect(result2.exceedsThreshold).toBe(true);
    });
    ...
    it('should trigger if either threshold is exceeded', () => {
      ...
      const result1 = checkPriceVariance(11.00, 10.00, 10, config);
      expect(result1.variancePercent).toBe(10); // Not exceeding (needs > 10)
      expect(result1.varianceAmount).toBe(10.00);
      expect(result1.exceedsThreshold).toBe(false);

      // Exceeds amount but not percentage
      const result2 = checkPriceVariance(10.60, 10.00, 100, config);
      expect(result2.variancePercent).toBe(6.00);
      expect(result2.varianceAmount).toBe(60.00);
  ...
  describe('Edge Cases', () => {
    it('should handle very small variances', () => {
      const result = checkPriceVariance(25.0001, 25.0000, 1000);

      expect(result.hasVariance).toBe(true);
      expect(result.variance).toBe(0.0001);
      expect(result.varianceAmount).toBe(0.10);
    });

    it('should handle large quantities', () => {
      const result = checkPriceVariance(25.50, 25.00, 10000);

      expect(result.varianceAmount).toBe(5000.00);
    });

    it('should handle zero period price (100% variance)', () => {
      const result = checkPriceVariance(10.00, 0, 100);

      expect(result.hasVariance).toBe(true);
      expect(result.variance).toBe(10.00);
      expect(result.variancePercent).toBe(100); // Any price from 0 is 100% increase
      expect(result.varianceAmount).toBe(1000.00);
    });

    it('should handle zero actual price (price decrease to 0)', () => {
      const result = checkPriceVariance(0, 25.00, 100);

      expect(result.hasVariance).toBe(true);
      expect(result.variance).toBe(-25.00);
      expect(result.variancePercent).toBe(-100);
      expect(result.varianceAmount).toBe(-2500.00);
    });
  ...
  describe('Decimal Precision', () => {
    it('should round variance to 4 decimal places', () => {
      const result = checkPriceVariance(25.123456, 25.000000, 100);

      expect(result.variance).toBe(0.1235); // Rounded to 4 decimals
    });

    it('should round variance percent to 2 decimal places', () => {
      const result = checkPriceVariance(25.333333, 25.000000, 100);

      expect(result.variancePercent).toBe(1.33); // Rounded to 2 decimals
    });

    it('should round variance amount to 2 decimal places', () => {
      const result = checkPriceVariance(25.123, 25.000, 100);

      expect(result.varianceAmount).toBe(12.30); // Rounded to 2 decimals
    });
  });

  describe('Error Handling', () => {
    it('should throw error for negative unit price', () => {
      expect(() => checkPriceVariance(-10, 25, 100))
        .toThrow('Invalid unit price: must be a positive finite number');
    });

    it('should throw error for negative period price', () => {
      expect(() => checkPriceVariance(25, -10, 100))
        .toThrow('Invalid period price: must be a positive finite number');
    });

    it('should throw error for zero quantity', () => {
      expect(() => checkPriceVariance(25, 25, 0))
        .toThrow('Invalid quantity: must be a positive finite number');
    });

    it('should throw error for negative quantity', () => {
      expect(() => checkPriceVariance(25, 25, -100))
        .toThrow('Invalid quantity: must be a positive finite number');
    });

    it('should throw error for NaN inputs', () => {
      expect(() => checkPriceVariance(NaN, 25, 100))
        .toThrow('Invalid unit price');

      expect(() => checkPriceVariance(25, NaN, 100))
        .toThrow('Invalid period price');

      expect(() => checkPriceVariance(25, 25, NaN))
        .toThrow('Invalid quantity');
    });

    it('should throw error for Infinity inputs', () => {
      expect(() => checkPriceVariance(Infinity, 25, 100))
        .toThrow('Invalid unit price');

      expect(() => checkPriceVariance(25, Infinity, 100))
        .toThrow('Invalid period price');

      expect(() => checkPriceVariance(25, 25, Infinity))
        .toThrow('Invalid quantity');
    });
  });
});
...
describe('createPriceVarianceNCR', () => {
  ...
  it('should store absolute value for NCR value field', async () => {
    ...
    const data: PriceVarianceNCRData = {
      ...
      expectedPrice: 25.00,
      variance: -0.50,
      varianceAmount: -50.00, // Negative variance
      createdBy: 'user-123',
    };
    ...
    const createCall = mockPrisma.nCR.create.mock.calls[0][0];
    expect(createCall.data.value).toBe(50.00); // Absolute value stored
  });
  ...
  it('should throw error for missing required fields', async () => {
    ...
    await expect(createPriceVarianceNCR(mockPrisma, invalidData as any))
      .rejects
      .toThrow('Missing required fields for NCR creation');
  });
});
...
describe('detectAndCreateNCR', () => {
  ...
  it('should not create NCR when variance below threshold', async () => {
    ...
    const result = await detectAndCreateNCR(
      mockPrisma,
      {
        ...
        itemCode: 'RICE-5KG',
        quantity: 100,
        unitPrice: 25.50, // Only 2% increase
        periodPrice: 25.00,
        createdBy: 'user-123',
      },
      config
    ...
    expect(result.varianceResult.hasVariance).toBe(true);
    expect(result.varianceResult.variancePercent).toBe(2.00);
  ...
  it('should work with Prisma.Decimal inputs', async () => {
    const result = await detectAndCreateNCR(mockPrisma, {
      ...
      unitPrice: new Prisma.Decimal(26.00),
      periodPrice: new Prisma.Decimal(25.50),
      createdBy: 'user-123',
    });
...
describe('validatePriceVarianceInputs', () => {
  it('should validate correct inputs', () => {
    const result = validatePriceVarianceInputs(25.00, 24.50, 100);
