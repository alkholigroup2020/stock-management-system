...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Get period prices for variance detection
    ...
    // Create a map of item_id -> period price
    ...
    // Get current stock levels for WAC calculation
    ...
    let message = "Delivery posted successfully.";
    if (result.ncrs.length > 0) {
      message = `Delivery posted. ${result.ncrs.length} price variance(s) detected and NCR(s) created automatically.`;
    }
    ...
    return {
      ...
      delivery: {
        ...
        posted_by: result.delivery.poster,
